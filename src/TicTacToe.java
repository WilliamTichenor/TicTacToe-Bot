...
public class TicTacToe {
    public static void main(String[] args) throws IOException {
        ...
        while (true)
        {
            ...
            {
                place=(int)Math.round(bestNetwork.calc(board,9-moves));
                //if (place<0) place=0;
                //if (place>8) place=8;
                while (board[place]!=-1)
                {
...
class TNetwork
{
    ...
    {
        return Math.pow(Math.E,d)/(Math.pow(Math.E,d)+1)-.0001;
    }
    public double customLogistic(double d,int spaces)
    {
        return spaces/(1+Math.pow(Math.E,-k*(d-xNaught)));
    }
    public double calc(int[] board,int spaces)
    {
        ...
        for (int i=0;i<5;i++)
        {
            ...
            hiddenVals[i]=customLogistic(hiddenVals[i],9);
        }
        for (int i=0;i<5;i++)
        {
        ...
        return customLogistic(finalVal,spaces);
    }
    ...
    public double calcFitness(TNetwork champ)
    {
        ...
        {
            nerf=true;
            place=(int)(champ.calc(board,9-moves));
            //if (place<0) place=0;
            //if (place>8) place=8;
            while (board[place]!=-1)
            {
        ...
        {
            if (nerf) moves++;
            fitness=Math.pow(moves*1.2-6,2)-2;
        }
        if (winner==0) fitness=30;
        if (winner==1)
        {
            moves--;
            fitness=Math.pow(15-moves*1.2,2.1)+10;
        }
        //if (nerf) fitness+=5;
